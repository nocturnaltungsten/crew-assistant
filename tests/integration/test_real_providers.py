...
def get_available_model(provider, exclude_embedding=True) -> Optional[str]:
    ...
    try:
        ...
        preferred_patterns = [
            ...
            "8b", "7b", "13b", "14b", "12b"  # Smaller model sizes
        ]
        
        # Avoid large models for testing
        avoid_patterns = [
            "30b", "32b", "27b", "70b", "65b", "180b",
            "embed", "embedding"
        ]
        ...
        for model in models:
            ...
            if (model.compatibility == "compatible" and 
                any(pattern in model.id.lower() for pattern in preferred_patterns)):
...
class TestRealLMStudioIntegration:
    ...
    async def test_real_async_chat(self, lmstudio_provider, available_model):
        ...
        assert isinstance(response, ChatResponse), "Should return ChatResponse"
        assert response.content, "Response should have content"
        assert "async" in response.content.lower() or "successful" in response.content.lower(), "Should contain expected content"
    ...
    async def test_real_streaming(self, lmstudio_provider, available_model):
        ...
        async for chunk in lmstudio_provider.chat_streaming(messages, available_model, max_tokens=50):
            chunks.append(chunk)
    ...
    def test_real_batch_processing(self, lmstudio_provider, available_model):
        ...
        requests = [
            ([ChatMessage(role="user", content="Say 'Request 1'")], available_model, {"max_tokens": 20}),
            ([ChatMessage(role="user", content="Say 'Request 2'")], available_model, {"max_tokens": 20}),
            ([ChatMessage(role="user", content="Say 'Request 3'")], available_model, {"max_tokens": 20})
        ]
    ...
    def test_real_model_compatibility_detection(self, lmstudio_provider):
        ...
        print(f"✅ Compatibility analysis: {compatible_count} compatible, {incompatible_count} incompatible, {unknown_count} unknown")
...
class TestRealOllamaIntegration:
    ...
    def test_real_ollama_chat(self, ollama_provider, ollama_model):
        ...
        messages = [
            ChatMessage(role="user", content="Say 'Hello from Ollama!' and nothing else.")
        ]
...
class TestRealProviderRegistry:
    ...
    def test_real_model_discovery(self):
        ...
        all_models = registry.list_all_models(
            ModelRequirements(compatibility_required=True)
        )
    ...
    def test_real_end_to_end_inference(self):
        ...
        # Get a model
        models = provider.list_models()
        compatible_models = [m for m in models if m.compatibility == "compatible" and "embed" not in m.id.lower()]
