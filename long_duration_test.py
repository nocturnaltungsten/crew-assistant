    ...
    async def execute(self, provider, model: str) -> Dict[str, Any]:
        ...
        try:
            response = await provider.chat_async(
                messages, model, 
                max_tokens=min(500, token_size // 10),
                temperature=0.7
            )
...
class StreamingTest(TestScenario):
    ...
    async def execute(self, provider, model: str) -> Dict[str, Any]:
        ...
        try:
            async for chunk in provider.chat_streaming(
                messages, model, max_tokens=1000
            ):
...
class LongDurationTestRunner:
    ...
    async def run_single_test(self) -> None:
        ...
        integration_logger.info(
            f"Starting {scenario.name} with {provider.name}:{model.id}"
        )
        ...
        try:
            ...
            if provider_key not in self.provider_stats:
                self.provider_stats[provider_key] = {
                    "requests": 0, "successes": 0, "total_time": 0
                }
            ...
            integration_logger.info(json.dumps({
                "event": "test_complete",
                "scenario": scenario.name,
                "provider": provider.name,
                "model": model.id,
            ...
            error_logger.error(
                f"Unexpected error in {scenario.name}: {str(e)}",
                exc_info=True
            )
    ...
    def log_summary(self) -> None:
        ...
        summary = {
            ...
            "tests_per_hour": (self.total_tests / runtime.total_seconds() * 3600) if runtime.total_seconds() > 0 else 0,
        ...
        current_model = self.test_models[self.current_model_idx][0].id if self.test_models else "None"
    ...
    async def run(self) -> None:
        ...
        print(f"🚀 Starting long duration test for {self.duration.total_seconds()/3600:.1f} hours")
        ...
        try:
            ...
            print(f"📈 Success rate: {(self.successful_tests/self.total_tests*100) if self.total_tests > 0 else 0:.1f}%")
