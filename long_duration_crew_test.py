...
class CrewWorkflowTest:
    ...
    async def execute(self, crew_engine, task_prompt: str) -> Dict[str, Any]:
        ...
        try:
            main_logger.info(f"Executing {self.complexity_level['level']} task: {task_prompt[:100]}...")
            ...
            workflow_logger.info(json.dumps({
                ...
                "validation_approved": "approved" in str(getattr(result, 'final_output', '')).lower(),
            }))
    ...
    def _analyze_result(self, result, execution_time: float, task_prompt: str) -> Dict[str, Any]:
        ...
        complexity_logger.info(json.dumps({
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "validation_approval_rate": (self.validation_approvals / self.execution_count * 100) if self.execution_count > 0 else 0,
            "workflow_completions": self.workflow_completions,
            "workflow_completion_rate": (self.workflow_completions / self.execution_count * 100) if self.execution_count > 0 else 0,
...
class LongDurationCrewTester:
    ...
    def __init__(self, duration_hours: float = 4.0):
        ...
        self.scenarios = [
            CrewWorkflowTest("Trivial Tasks", TaskComplexity.TRIVIAL, TaskComplexity.TRIVIAL["weight"]),
            CrewWorkflowTest("Simple Tasks", TaskComplexity.SIMPLE, TaskComplexity.SIMPLE["weight"]),
            CrewWorkflowTest("Moderate Tasks", TaskComplexity.MODERATE, TaskComplexity.MODERATE["weight"]),
            CrewWorkflowTest("Complex Tasks", TaskComplexity.COMPLEX, TaskComplexity.COMPLEX["weight"]),
            CrewWorkflowTest("Vague Tasks", TaskComplexity.VAGUE, TaskComplexity.VAGUE["weight"]),
            CrewWorkflowTest("Vague Override Tasks", TaskComplexity.VAGUE_WITH_OVERRIDE, TaskComplexity.VAGUE_WITH_OVERRIDE["weight"]),
        ]
    ...
    def setup_crew_engine(self, provider_name: str, model: str) -> bool:
        ...
        try:
            ...
            self.crew_engine = create_crew_engine(
    ...
    def auto_select_lm_studio_model(self) -> Optional[str]:
        ...
        try:
            ...
            preferred_patterns = [
                "qwen", "llama", "mistral", "deepseek", "phi", "gemma"
            ]
            ...
            chat_models = [m for m in model_names if "embed" not in m.lower() and "embedding" not in m.lower()]
    ...
    async def log_summary(self):
        ...
        summary = {
            ...
            "overall_success_rate": (self.total_successes / self.total_tests * 100) if self.total_tests > 0 else 0,
        ...
        main_logger.info(f"SUMMARY: {self.total_tests} tests, {self.total_successes} successes "
        ...
        print(f"📊 Tests: {self.total_tests} | Successes: {self.total_successes} ({summary['overall_success_rate']:.1f}%)")
        print(f"🤖 Current Model: {self.current_model}")
        print(f"📈 Scenarios tested:")
        for scenario in self.scenarios:
            stats = scenario.get_stats()
            print(f"   {stats['complexity_level']:12} | {stats['executions']:3} tests | {stats['success_rate']:5.1f}% success")
        print(f"{'='*60}\n")
    ...
    def _generate_insights(self) -> Dict[str, Any]:
        ...
        if insights["validation_patterns"]:
            vague_approval = insights["validation_patterns"].get("vague", {}).get("approval_rate", 0)
            vague_override_approval = insights["validation_patterns"].get("vague_override", {}).get("approval_rate", 0)
            ...
            simple_success = insights["validation_patterns"].get("simple", {}).get("success_rate", 0)
            complex_success = insights["validation_patterns"].get("complex", {}).get("success_rate", 0)
...
async def main():
    ...
    parser.add_argument('duration', type=float, default=4.0, nargs='?',
