    ...
    def get_tool_definitions_prompt(self) -> str:
        ...
        for tool in tools:
            ...
            for param in tool.parameters:
                ...
                default_str = f" (default: {param.default})" if not param.required and param.default else ""
                tool_desc.append(f"  • {param.name} ({param.type}) - {required_str}: {param.description}{default_str}")
    ...
    def execute_tool_calls(self, response_text: str) -> List['ToolResult']:
        ...
        if self.config.verbose:
            if parse_result.tool_calls:
                ...
                print(f"🔍 Parser found no tool calls in response")
        ...
        for i, tool_call in enumerate(parse_result.tool_calls):
            ...
            if not self.is_tool_allowed(tool_call.tool_name):
                from .tools import ToolResult, ToolCallStatus
                result = ToolResult(
    ...
    def execute_task(self, context: TaskContext) -> AgentResult:
        ...
        try:
            ...
            if self.config.verbose:
                ...
                print(f"🤖 {self.config.role} completed task in {execution_time:.2f}s (tools: {tool_count})")
